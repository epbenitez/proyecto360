 ...
 */
public class CargosBO extends XlsLoaderBO {

    private Service service;

    public CargosBO(Service service) {
    ...
    public String getIdentificador() {
        Date d = new Date();
        return "C" + UtilFile.dateToString(d, "yyyyMMddhhmmss");
    }
            ...
            try {
                ...
                for (ConfiguracionExcel ce : this.configuracionExcel) {
                    ...
                    if (monto != null && monto.intValue() != 0) {
                        ...
                        cargo.setConcepto(ce.getConcepto());
                        cargo.setDepartamento(dpto);
                        cargo.setFechaPeriodo(fecha);
                        cargo.setMonto(monto);
                        cargo.setIdentificadorcarga(this.identificador);

                        service.getCargoDao().save(cargo);
    ...
    public void revertirCargos(String identificadorCarga){
        service.getCargoDao().elimina(identificadorCarga);
    }
